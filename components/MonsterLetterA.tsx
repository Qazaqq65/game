...
interface MonsterLetterAProps {
  /** Дисплей символы (А немесе а). */
  ch?: string;
  size: number;
  color: string;
  strokeColor: string;
...
/** «А / а»: заливка + узор по клипу; при drag — градиент. Глаза и рот как у остальных букв (пресет А). */
export function MonsterLetterA({
  ch = "А",
  size,
  color,
  strokeColor,
  ...
  const clipId = `m-letter-A-clip-${svgUid}`;
  const gradId = `m-letter-A-grad-${svgUid}`;

  // PRESETS["А"] — те же координаты, что в MonsterLetter
  const eyeY = size * 0.52;
  const eyeLX = size * 0.42;
  const eyeRX = size * 0.58;
    ...
    <motion.div
      ...
      animate={
        isDragging
          ? dragOuterStill
          : isHovered
            ? { y: 0, rotate: [0, -2, 2, 0], scaleX: [1, 1.02, 1], scaleY: [1, 0.99, 1] }
            : isSnapped
              ? { y: 0, rotate: 0, scaleX: [1, 1.05, 0.98, 1], scaleY: [1, 0.96, 1.03, 1] }
              : isLowEnd
                ? { y: 0, rotate: 0, scaleX: 1, scaleY: 1 }
                : { y: 0, rotate: [0, -0.6, 0.6, 0], scaleX: 1, scaleY: 1 }
      }
      transition={
        isDragging
          ...
          : isHovered
            ...
            : isSnapped
              ...
              : isLowEnd
                ? { duration: 0.12 }
                : { duration: 2.2, repeat: Infinity, repeatDelay: 1.5 }
      }
    >
      <svg
      ...
      >
        <defs>
          <linearGradient id={gradId} x1="0%" y1="0%" x2="100%" y2="100%">
            ...
            <stop offset="45%" stopColor={strokeColor} stopOpacity={0.95} />
        ...
        <g clipPath={`url(#${clipId})`}>
          <rect width={size} height={size} fill={isDragging ? `url(#${gradId})` : color} />
          {!isDragging && !noPattern ? (
            <Pattern pat={pat} pc={strokeColor} size={size} />
          ) : null}
        </g>
